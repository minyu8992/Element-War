...
def element_dataset(n=8000, element_name='fire'):
    df = pd.read_csv(f"element/{element_name}.tsv", sep='\t')
...
def get_dataset(name, n=8000):
    ...
    elif name in ["fire", "water", "rock", "lightning", "wind"]:
        return element_dataset(n, name)
    else:
